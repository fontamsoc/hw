...
`eX_JumpOrBranch_i` is resolved in EX from the decoded control and the dedicated branch comparator (`eX_eq_i`/`eX_brLt_i`/`eX_brLtu_i`), plus JALR, FENCE.I, ERET, prediction-miss and exception conditions. In the same cycle it redirects fetch (`iF_pc`, via `iF_eX_JumpOrBranch_i` and `eX_JumpOrBranchAddr_i`) and flushes the mis-fetched instruction(s) through `_iF_flushed`, which also suppresses their scoreboard lock. Mispredicted branches/JAL/RET and exceptions take the same redirect path, and it costs two issue slots: the resolving cycle, whose fetch was already launched, and the cycle spent re-reading the i-cache from the freshly captured `iF_pc`.

`PUEARLYREDIRECTFETCH` recovers the second slot for the branch cause alone. In the resolving cycle the i-cache index and tag, and the branch-prediction-table index, are driven from the anti-prediction of the two registered candidates (`iD_pc_plus_INSNBITSzBy8` / `iD_pc_plus_iD_Bimm`) rather than from `iF_pc_i`, and the fetch that read produces is exempted from the flush -- so the target enters decode one cycle earlier. `iF_pc` is untouched and still captures the same address through `eX_JumpOrBranchAddr_i`, which is what keeps the fetch miss controller, `excEpc`/`excTval` and the return-address stack unchanged. The exemption is withdrawn when an exception is taken in that same cycle, as `iF_pc` then goes to the trap vector while the read went to the branch target. JALR, RET, FENCE.I, ERET and exceptions keep the two-cycle path, their targets not being registered candidates.

### I-cache miss and refill

An i-cache miss holds fetch for four issue slots: the miss verdict, presenting the request on the shared bus, the response clockcycle, in which the refilled word is written into the i-cache, and the probe that reads it back out. While the refill is in flight the re-read of the missing pc is withheld (`iCache_re_w`), the brams holding the miss verdict on their outputs, so the fill write lands on a settled array instead of colliding with a same-set read that the `no_rw_check` brams would be licensed to resolve arbitrarily.

`PUICACHEFILLBYPASS` recovers the probe slot: the refilled word is captured beside the fill write and presented in place of the i-cache read for the one clockcycle the fetch stage consumes it, with the hit forced for that clockcycle only, so a miss costs three slots instead of four. The capture is qualified by the response being for the word still being fetched (`iF_mem_addr` against `iF_pc`), by the i-cache not already holding it, and by no redirect landing in that clockcycle; it is held until consumed, so a stalled decode costs no more than an ordinary hit, and cleared by any redirect. The miss state machine never sees the forced hit, and nothing bus-facing changes.
