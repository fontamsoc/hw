...
`include "dev/sdcard/sdcard_sdio.v"
...
wire            sdcmd_i_w, sdcmd_o_w, sdcmd_e_w;
wire [4 -1 : 0] sddat_i_w, sddat_o_w, sddat_e_w;
...
always @ (posedge clk_2x_w) begin
	...
	end else if ((sdactivity || litedram_init_error)) begin
		activity <= 1;
...
localparam S_WBPI_SERIAL1      = (S_WBPI_BOOTLDR + 1);
localparam S_WBPI_INVALIDDEV = (S_WBPI_SERIAL1 + 1);
...
localparam WBPI_WORDBITSZ         = 64/* RAM WORDBITSZ */;
...
localparam IRQ_SERIAL   = (IRQ_GP1IO + 1);
localparam IRQ_SERIAL1  = (IRQ_SERIAL + 1);

localparam IRQSRCCOUNT = (IRQ_SERIAL1 +1); // Number of interrupt source.
...
cpu #(
	...
	,.CLKFREQ       (CLK2XFREQ)
	,.ICACHESETCNT  ((1024/(WBPI_WORDBITSZ/8))*((ICACHESZ/ICACHEWAYCNT)/PUCOUNT))
	,.DCACHESETCNT  ((1024/(WBPI_WORDBITSZ/8))*((DCACHESZ/DCACHEWAYCNT)/PUCOUNT))
	,.TLBSETCNT     (TLBSZ)
...
//tbuf sdcmd_tbuf  (.i (sdcmd_o_w),    .o (sdcmd_i_w),    .e (sdcmd_e_w),    .io (sd_di));
//tbuf sddat0_tbuf (.i (sddat_o_w[0]), .o (sddat_i_w[0]), .e (sddat_e_w[0]), .io (sd_do));
//tbuf sddat1_tbuf (.i (sddat_o_w[1]), .o (sddat_i_w[1]), .e (sddat_e_w[1]), .io (sd_dat1));
//tbuf sddat2_tbuf (.i (sddat_o_w[2]), .o (sddat_i_w[2]), .e (sddat_e_w[2]), .io (sd_dat2));
//tbuf sddat3_tbuf (.i (sddat_o_w[3]), .o (sddat_i_w[3]), .e (sddat_e_w[3]), .io (sd_cs));

IOBUF sdcmd_tbuf  (.I (sdcmd_o_w),    .O (sdcmd_i_w),    .T (~sdcmd_e_w),    .IO (sd_di));
IOBUF sddat0_tbuf (.I (sddat_o_w[0]), .O (sddat_i_w[0]), .T (~sddat_e_w[0]), .IO (sd_do));
IOBUF sddat1_tbuf (.I (sddat_o_w[1]), .O (sddat_i_w[1]), .T (~sddat_e_w[1]), .IO (sd_dat1));
IOBUF sddat2_tbuf (.I (sddat_o_w[2]), .O (sddat_i_w[2]), .T (~sddat_e_w[2]), .IO (sd_dat2));
IOBUF sddat3_tbuf (.I (sddat_o_w[3]), .O (sddat_i_w[3]), .T (~sddat_e_w[3]), .IO (sd_cs));
...
sdcard_sdio #(
	...
	,.sdclk_o (sd_sclk)
	,.sdcmd_i (sdcmd_i_w) ,.sdcmd_o (sdcmd_o_w) ,.sdcmd_e (sdcmd_e_w)
	,.sddat_i (sddat_i_w) ,.sddat_o (sddat_o_w) ,.sddat_e (sddat_e_w)
...
localparam RAMCACHEWAYCNT = 2;
localparam RAMCACHESZ = /* In (WORDBITSZ/8) units */
	((1024/(WORDBITSZ/8))*(32/RAMCACHEWAYCNT));
...
serial_uart #(
	...
	,.wb_cyc_i   (s_wbpi_cyc_w[S_WBPI_SERIAL1])
	,.wb_stb_i   (s_wbpi_stb_w[S_WBPI_SERIAL1])
	,.wb_we_i    (s_wbpi_we_w[S_WBPI_SERIAL1])
	,.wb_addr_i  (s_wbpi_addr_w[S_WBPI_SERIAL1])
	,.wb_sel_i   (s_wbpi_sel_w[S_WBPI_SERIAL1])
	,.wb_dat_i   (s_wbpi_dato_w[S_WBPI_SERIAL1])
	,.wb_bsy_o   (s_wbpi_bsy_w[S_WBPI_SERIAL1])
	,.wb_ack_o   (s_wbpi_ack_w[S_WBPI_SERIAL1])
	,.wb_dat_o   (s_wbpi_dati_w[S_WBPI_SERIAL1])
	,.wb_mapsz_o (s_wbpi_mapsz_w[S_WBPI_SERIAL1])

	,.irq_stb_o (irq_src_stb_w[IRQ_SERIAL1])
	,.irq_rdy_i (irq_src_rdy_w[IRQ_SERIAL1])

	,.rx_i (uart1_rx)
	,.tx_o (uart1_tx)
);

assign dev_id_w    [S_WBPI_SERIAL1] = 5;
assign dev_useirq_w[S_WBPI_SERIAL1] = 1;
...
dcache #(
	...
	,.CACHESETCNT (RAMCACHESZ/(WBPI_WORDBITSZ/WORDBITSZ))
	,.CACHEWAYCNT (RAMCACHEWAYCNT)
